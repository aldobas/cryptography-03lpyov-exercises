...
if __name__ == "__main__":
    ...
    print(m.state)
    print(m.digest())
    print(_encode(m.state, md5.digest_size)) # different, the digest is computed after the padding has been added

    # ------------------------------------------------------------------
    # state__ =  (3708274669, 943078308, 721761903, 529810996)
    #
    # print("state__2")
    # m2 = md5(state=_encode(state__,md5.digest_size),count=512)
    # m2.update(to_add)
    # print(m2.hexdigest())
    # ------------------------------------------------------------------

    print("attack")
    print("state from digest()")
    m2 = md5(state=m.digest(),count=512)
    m2.update(to_add)
    print(m2.hexdigest())

    print("full message: check correctness")
    m = md5()
    m.update(secret + message + pad + to_add)
    print(m.hexdigest())
