...
###############################################
# guessing the first byte of the keystream
###############################################
print("Guess first byte")
#an array to count the ASCII letter generated after decrypting with a given key, the key is the vector index
counter = numpy.zeros(256,dtype=int)
# print(counter)

for key_byte in range(0,256):
    for c in ciphertexts:
        if chr(c[0] ^ key_byte) in ascii_uppercase: #ascii_lowercase / printable / ascii_uppercase
            counter[key_byte] +=1
...
#prune the set by only including the candidate bytes that
maxm = []
for i in range(256):
    if counter[i] == m:
...
# prepare to store all the candidates for all the bytes of the keystream
...
###############################################
print("--------------------------------------")
print("Guess the second byte")
counter = numpy.zeros(256,dtype=int)

for key_byte in range(0,256):
    for c in ciphertexts:
        if chr(c[1] ^ key_byte) in punctuation+ascii_lowercase: #ascii_lowercase / printable / ascii_uppercase / punctuation
            counter[key_byte] +=1
print(counter)
maxm = []
for i in range(256):
...
#
#assessment of the trials needed with a restricted brute force
#
print("--------------------------------------")
print("candidates' stats")
i = 0
tot = 1
for c in candidate_key_byte:
    # print(str(i)+"-->"+str(c))
    i+=1
    tot*=len(c)
    if i > 29:
...
##################################################################################
# Guess the first byte, but let's use frequencies now
##################################################################################
print("--------------------------------------")
print("Guess the first byte with frequencies")
...
print(ordered_freqs)
# print(ordered_freqs[0])
# print(chr(ciphertexts[0][0] ^ ordered_freqs[0][1]))
...
#correct after observation ciphertexts # 1
new_keystream = bytearray()
delta = bytes([ord('i')^ord('I')^keystream[0]])
new_keystream+=delta
...
s1 = b'he, too, has been changed in he7 x    '
s2 = b'I have passed with a nod of thidhi46'

print(s1.index(b'7'))
# print(s1[24:].index(b'd'))
# new_keystream = bytearray()
# delta = bytes([])
print(new_keystream[31])
new_keystream[31] = ord('7')^ord('r')^keystream[31]
print(new_keystream[31])
# new_keystream+=keystream[1:]
for c in ciphertexts:
    print(bytes_xor(c,new_keystream))
...
######################################
# correct after observation new set
# new_keystream = bytearray()
# new_keystream += bytes([148])
# delta = bytes([ord('1')^ord('y')^keystream[1]])
# new_keystream += delta
# delta = bytes([ord('e')^ord(' ')^keystream[2]])
# new_keystream += delta
